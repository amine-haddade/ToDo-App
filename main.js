...
function addTsk(){
    if(inputBox.value === ''){  // si usr taper une tache vide
        ...
        listContainer.appendChild(li);// <<<enfant>>> ajouter l'elment varible li dans une varible listContiner

        let span= document.createElement("span");
        span.innerHTML="\u00d7";  // icone 
        li.appendChild(span) // dans elment li ajouet une child span

    }
    inputBox.value="";// vider  input une fois add button
    savedeta()
}
listContainer.addEventListener("click"/* -> evnt */,function(e){  // Lorsque clicker sur elemnt li applicer le block suivant

    if(e.target.tagName==="LI"){ // name de elment est li create name dans html
